...
package object nodescala {
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = {
      try { Await.result(f, Duration.Zero) }
      catch { case _: TimeoutException => throw new NoSuchElementException }
    }
     ...
     *  The resulting future contains a value returned by `cont`.
     */
    def continue[S](cont: Try[T] => S): Future[S] = f.map(result => cont(Try(result)))
