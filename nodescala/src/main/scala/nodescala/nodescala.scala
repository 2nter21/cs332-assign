...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    try {
      response.foldLeft(()) { (_, part) =>
        if (token.nonCancelled) exchange.write(part)
        else return
      }
    } finally {
      exchange.close()
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val listenerStartSubscription = listener.start()

    val requestHandlingSubscription = Future.run() { cancellationToken =>
      async {
    ...
    Subscription(listenerStartSubscription, requestHandlingSubscription)
  }
